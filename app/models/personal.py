...
class Personal(Base):
    ...
    sexo                 = Column(Enum(SexoEnum, name="sexo_tipo",
                                values_callable=enum_values(SexoEnum)),
                                nullable=False)
    fecha_nacimiento     = Column(Date, nullable=False)
    estado_civil         = Column(Enum(EstadoCivilEnum, name="estado_civil_tipo",
    ...
    afp_nombre           = Column(Enum(AfpEnum, name="afp_tipo",
                                values_callable=enum_values(AfpEnum)))
    ...
    # ── Relaciones ─────────────────────────────────────────
    datos_laborales     = relationship("DatosLaborales",     back_populates="personal",
                                uselist=False, cascade="all, delete-orphan")
    familiares          = relationship("Familiar",           back_populates="personal",
                                cascade="all, delete-orphan")
    formacion_academica = relationship("FormacionAcademica", back_populates="personal",
                                cascade="all, delete-orphan")
    otros_estudios      = relationship("OtrosEstudios",      back_populates="personal",
                                cascade="all, delete-orphan")
    experiencia_laboral = relationship("ExperienciaLaboral", back_populates="personal",
                                cascade="all, delete-orphan")
    experiencia_docente = relationship("ExperienciaDocente", back_populates="personal",
                                cascade="all, delete-orphan")
    otras_instituciones = relationship("OtrasInstituciones", back_populates="personal",
                                uselist=False, cascade="all, delete-orphan")
    reconocimientos     = relationship("Reconocimiento",     back_populates="personal",
                                cascade="all, delete-orphan")
...
class DatosLaborales(Base):
    ...
    id                  = Column(Integer, primary_key=True, index=True)
    personal_id         = Column(Integer, ForeignKey("personal.id", ondelete="CASCADE"),
    ...
    condicion           = Column(Enum(CondicionEnum, name="condicion_tipo",
...
class Familiar(Base):
    ...
    apellido_materno    = Column(String(60),  nullable=False)
    nombres             = Column(String(100), nullable=False)
    parentesco          = Column(Enum(ParentescoEnum, name="parentesco_tipo",
